<template>
  <div>
    <h2>{{ title }}</h2>
    <carousel :responsive="{0:{items:1}}" :autoplay="true" :loop="true" :items="5" :autoplaySpeed="1400" :dots="false" :nav="false" :margin="5"
      ...
      <div class="w-full h-52 overflow-hidden relative" v-for="row in rows">
        <a :href="'etkinlikler/' + row.slug">
          ...
          <div class="z-10 bg-theme-color opacity-95 hover:opacity-100 p-2 absolute bottom-0 right-0 left-0">Şimdi Senin
            Zamanın !
          </div>
        </a>
      </div>
